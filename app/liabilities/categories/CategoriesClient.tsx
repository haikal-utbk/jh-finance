...
import { addLiabilityCategory, deleteLiabilityCategory } from "./actions";
...
export default function CategoriesClient({ categories }: { categories: Category[] }) {
  ...
  async function handleSubmit(formData: FormData) {
    ...
    setError(null);
    try {
      await addLiabilityCategory(formData);
  ...
  async function handleDelete(id: number) {
    ...
    setDeletingId(id);
    try {
      await deleteLiabilityCategory(id);
    } catch (err: any) {
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl">Kategori Kewajiban</h1>
        <Link href="/liabilities" className="text-sm text-moss font-medium">
          &larr; Kembali ke Kewajiban
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <form ref={formRef} action={handleSubmit} className="card space-y-4 h-fit">
          ...
          <div>
            ...
            <input name="name" className="input" placeholder="Contoh: Pinjaman Koperasi" required />
          </div>
